...
public class PdsInputOkCommand implements PdsInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		while(fileNames.hasMoreElements()) {
			...
			if(multipartRequest.getFilesystemName(file) != null) { //들어오는 파일의 시스템 이름이 널값이 아닐때 수행.
				originalFileName += multipartRequest.getOriginalFileName(file) + "/";
				filesystemName += multipartRequest.getFilesystemName(file) + "/";
			}
		}
		originalFileName = originalFileName.substring(0, originalFileName.length()-1); // 마지막 슬래시 제거
		filesystemName = filesystemName.substring(0, filesystemName.length()-1); // 마지막 슬래시 제거
		...
		// 앞에서 전성된 자료와 가공된 자료들을 모두 VO에 담아서 DB에 저장 할 수 있도록 한다.
		PdsVO vo = new PdsVO();
		vo.setMid(mid);
